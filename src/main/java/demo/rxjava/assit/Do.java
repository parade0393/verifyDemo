 ...
 * 订阅开始后执行的第一个方法是onSubscribe，它永远是当前线程不受任何操作符的影响
 ...
 * 2.doOnNext和doAfterNext和onNext的执行顺序第一次doOnNext>onNext>doAfterNext，但是若在doAfterNext之后有切换线程(调用ObserveOn())那么doAfterNext就会按照代码的上下顺序执行了,如果有多个doAfterNext，则从后向前执行
 * 3.doOnNext>onNext>doAfterNext>doOnComplete>onComplete
 * 4.doOnError>doError
 * 5.doOnComplete>onComplete
 * 6.onNext>doAfterNext>doOnTerminate>onComplete>doFinally
 * 7.doOnTerminate>onError>doFinally
 */
public class Do {
...
//                emitter.onComplete();
                ...
                .doOnEach(new Consumer<Notification<Integer>>() {
                    ...
                    public void accept(Notification<Integer> integerNotification) throws Throwable {
                        System.out.println("doOnEach执行了:"+integerNotification.toString());
                    }
                })
                .doOnComplete(new Action() {
                    @Override
                    public void run() throws Throwable {
                        System.out.println("doOnComplete执行了");
                    }
                })
                .doOnError(new Consumer<Throwable>() {
                    @Override
                    public void accept(Throwable throwable) throws Throwable {
                        System.out.println("doOnError执行了:"+throwable.getMessage());
                    }
                })
                ...
                .doOnTerminate(new Action() {
                    @Override
                    public void run() throws Throwable {
                        System.out.println("doOnTerminate执行了");
                    }
                })
            ...
            @Override
            public void accept(Integer integer) throws Throwable {
                System.out.println("第二次doAfterNext:" + Thread.currentThread().getName());//第二次onNextRxComputationThreadPool-1
            }
        }).doOnNext(new Consumer<Integer>() {
            @Override
            public void accept(Integer integer) throws Throwable {
